...
namespace Worktop.Core.Services.Interfaces
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(UserManager<User> userManager, SignInManager<User> signInManager, IEmailSender emailSender, IRolesService rolesService,
        ...
        public async Task<User> Register(string email, string username, string password)
        {
            ...
            if ((await userManager.CreateAsync(user, password)).Succeeded)
            {
                ...
                string callbackUrl = $"{Configuration.GetValue<string>(AppSettingsKeys.ServerAddress)}Auth/ConfirmAccount?email={user.Email}&token={confirmAccountToken}";

                await emailSender.Send(Constants.ActivationAccountEmail(email, callbackUrl));
